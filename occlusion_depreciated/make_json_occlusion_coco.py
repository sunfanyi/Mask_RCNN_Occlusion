...
def write_to_json(train_val_split=False, debug=False, subset=None, write=True):
    """
    train_val_split:
        folders names represent occlusion_depreciated level, for example
        ...
        different occlusion_depreciated level, and therefore have to be in either training
        ...
        for lst in os.listdir(lists_dir):
            if subset is not None:
                if all(occ_level not in lst for occ_level in subset):
                    # ignore the unwanted occlusion_depreciated level
                    print('Unwanted occlusion_depreciated level: {}'.format(lst))
                    num_folders += 1
                    continue
                else:
            ...
            if num_folders // folders_per_cat:  # finish browsing one category
                ...
                print('\nStart category: {}'.format(par_dir.split('FGL')[0]))

            print('extracting images from: {}...'.format(par_dir))
            num_train = 0
            num_val = 0
...
if __name__ == "__main__":
    ...
    subset = ['FGL1_BGL1']
    res = write_to_json(train_val_split=True, debug=True,
                        subset=subset, write=False)
